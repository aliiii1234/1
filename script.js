...
function saveProgram() {
  ...
  rows.forEach((row) => {
    ...
    const overload = row.children[4].querySelector("input").value;
  ...
  if (sessionIndex !== '') {
    ...
    data.exercises.forEach((exercise) => {
      ...
      newRow.innerHTML = `
        ...
        <td><input type="text" value="${exercise.overload}" class="overload-input" oninput="autoResize(this)"></td>
...
function addRow() {
  ...
  newRow.innerHTML = `
    ...
    <td><input type="text" placeholder="اضافه بار" class="overload-input" oninput="autoResize(this)"></td>
    <td><button type="button" id="deleteButton"onclick="removeRow(this)"></button></td>
  `;
  tableBody.appendChild(newRow);
  ...
  autoResize(restInput); // تنظیم عرض ورودی اضافه بار
}
